...
**项目名称**：Axis CLI（自驱式 CLI）  
**项目定位**：面向自治 AI Agent 原生设计的自驱式命令行工具  
**核心创新**：将「人类操作主体」封装为 Agent 可无差别调用的自然语言函数  
**终态目标**：实现 Agent 主导的「数字 + 物理」全流程自治闭环  
**技术栈**：Go 1.21+  
**当前状态**：框架设计完成，未开始代码实现

**项目白皮书**：详见 `docs/WHITEPAPER.md`  

## ⚠️ 重要警告：禁止直接编码实现

**在开始任何编码工作之前，必须先完成以下步骤：**

1. **完整阅读项目白皮书**（docs/WHITEPAPER.md）- 理解终态定位
2. **完整阅读所有架构文档**（docs/architecture/ 和 docs/protocols/）
3. **理解核心设计理念**（Human-as-a-Function、统一任务调度、结构化指令解析）
4. **确认当前里程碑目标**（里程碑 1：极简版，聚焦核心能力验证）
5. **与原设计者确认理解无误后，再开始实施**
...
**正确的工作流程：**
1. 阅读白皮书 → 理解终态 → 阅读架构文档 → 理解核心能力 → 提问确认 → 设计审查 → 编码实现 → 测试验证

## 项目目标

实现自驱式 CLI（Self-Driving CLI），打通 Agent 数字世界调度能力与物理世界执行能力，实现「Agent 主导、人类协同、机器执行」的全流程自治闭环。

**核心能力**（基于白皮书定义）：
- Human-as-a-Function：将人类封装为 Agent 可调用的自然语言函数
- 统一任务调度：工具和人类在同一调度队列
- 结构化指令解析：兼容人类命令与 Agent 原生指令
- 上下文持久化：跨任务、跨会话的上下文透传
- 结果结构化解析：自然语言回复 → 结构化数据
...
### main 分支（极简方案 - 当前）
采用**极简里程碑 1**策略，聚焦核心能力验证，只实现白皮书定义的不可替代价值：

**里程碑 1 极简范围**（必须保留的核心能力）：
- ✅ Human-as-a-Function 协议层（请求/响应、错误码、超时）
- ✅ 统一任务调度（FIFO、超时控制、重试）
- ✅ 上下文持久化（内存队列）
- ✅ 结构化指令解析（兼容人类和 Agent 指令）
- ✅ 结果结构化解析（自然语言 → 结构化数据）
- ✅ 简单的 CLI 入口（使用 cobra 框架）

**暂不实现**（移至里程碑 2 或保留在 feature/full-architecture 分支）：
- ❌ 全局共享层（事件总线、数据管道、锁服务）
- ❌ 契约层（准入规则、SLA、异常码）- 里程碑 2
- ❌ DAG 并行调度 - 里程碑 2
- ❌ 管控者架构 - 里程碑 2
- ❌ 分级左移（Level 1/2/3）- 保留在 feature/full-architecture
- ❌ 三层存储（Layer 2/3）- 保留在 feature/full-architecture
...
### 立即开始（里程碑 1 - 极简版）
**实施顺序**（按白皮书核心能力优先级）：
1. 实现 Human-as-a-Function 协议层（核心创新）
   - 请求/响应结构定义
   - 错误码体系
   - 超时管控机制
2. 实现统一任务调度（FIFO + 内存队列）
   - 任务队列（内存实现）
   - 超时控制
   - 重试策略
3. 实现上下文持久化（内存队列）
...
5. 实现结果结构化解析
   - 自然语言回复 → 结构化数据
   - 基础解析逻辑
6. 实现简单的 CLI 入口（使用 cobra 框架）
   - 基础命令解析
   - 信号处理

### 测试策略（极简版）
- 核心能力必须有单元测试
- Human-as-a-Function 协议层测试覆盖率 ≥ 80%
- 基础端到端集成测试验证通过

## 重要注意事项

### 必须遵守的铁律
1. **核心能力不可简化**：Human-as-a-Function、统一任务调度、结构化指令解析、上下文持久化、结果结构化解析
2. **CLI 框架可使用成熟库**：允许使用 cobra 等成熟 CLI 框架（基础设施依赖）
3. **业务逻辑零外部依赖**：核心业务逻辑只使用 Go 标准库
