...
> "通过实践而发现真理，又通过实践而证实真理和发展真理。从感性认识而能动地发展到理性认识，又从理性认识而能动地指导革命实践。"

在开发中的映射：Phase III 执行（实践）→ A7 Distill（认识）→ A8 Sublate 写回规则（理性认识）→ 下一轮 Phase III 用新规则指导执行（再实践）。认识不是一次完成的，规则也不是一次写对的。

**二、主要矛盾与矛盾的主要方面（《矛盾论》）**

> "在复杂的事物的发展过程中，有许多的矛盾存在，其中必有一种是主要的矛盾，由于它的存在和发展，规定或影响着其他矛盾的存在和发展。"

在开发中的映射：每次 Phase 声明中的"主要矛盾侧面"就是这个——不是列出所有问题，而是识别当前阶段的主要矛盾，集中力量解决它。其他矛盾在主要矛盾解决后会自然转化或消失。

**三、调查研究，反对教条主义（《反对本本主义》）**

> "没有调查，没有发言权。"
> "中国革命斗争的胜利要靠中国同志了解中国情况。"

在开发中的映射：不读代码不发言（investigate_before_answering）。不套模板（P4 分解未知）。每个项目有自己的"中国情况"——照搬其他项目的架构是教条主义。

**四、对立统一规律**

> "矛盾着的对立的双方互相斗争的结果，无不在一定条件下互相转化。"

在开发中的映射：稳定与发展不是非此即彼，而是在条件变化时互相转化。永久条款在前提崩塌时可以变为渐进条款；过渡条款在被反复验证后可以提升为永久条款。§13.1 的三分类不是静态标签，是动态平衡。

**五、从群众中来，到群众中去（《关于领导方法的若干问题》）**

> "将群众的意见集中起来，又到群众中去作宣传解释，化为群众的意见，使群众坚持下去。"

在开发中的映射：L3 实践中涌现的模式（群众意见）→ 提炼为 L2 原则（集中）→ 写入 L1 约束后指导下一轮 L3 执行（到群众中去）。§13.4 模式 C 就是这个过程的机制化。
...
| 毛选术语 | 体系对应概念 | 应用场景 |
|----------|-------------|----------|
| 主要矛盾 | Phase 声明中的"主要矛盾侧面" | 每次工作开始时的问题聚焦 |
| 实践论循环 | 反馈闭环（L3→L2→L1→L3） | §13.5 规则迭代机制 |
| 扬弃（Aufhebung） | A8 Sublate 流程 | 规则进化方法 |
| 调查研究 | Phase I Objectification + investigate_before_answering | 实践信息收集 |
| 教条主义 | 套模板、不读代码直接生成 | P4 反面案例 |
| 从群众中来到群众中去 | §13.4 模式 C（实践突破理论） | 规则从实践涌现 |
| 对立统一 | §13.1 条款稳定性动态转化 | 永久/渐进/过渡不是静态标签 |
| 矛盾转化 | Phase 转换（I→II→III→I） | 主要矛盾解决后新矛盾上升 |
