...
**六、虚构是意图的提前对象化，方向性价值大于正确性价值**

Axis 的 L2 文档中存在大量"虚构内容"（error-code-conventions 描述不存在的错误码体系，kernel-abstraction-model 描述不存在的 WAL）。这些不是错误，是设计意图在实现之前就获得了客观存在——完成了 Phase I（Objectification）但跳过了 Phase II 和 III。

虚构内容的价值不在正确性（具体设计可能全错），在方向性（它告诉我们作者认为什么是重要的）。正确的扬弃方式：保留方向性命题 + 声明激活条件 + 删除过早的具体实现细节。

推论：§13.4 模式 A（理论超前实践）的处置不是"标记后放着"，而是"提炼方向 + 声明激活条件 + 删除实现"。
...
Axis 实践证据：
- Staged Evolution 的 promote/discard 是演化的原子操作——不是"加新功能"，是"验证后替换旧结构"
...
| 虚构是意图的提前对象化 | Objectification 的时序独立性 | L2 虚构内容是 Phase I 的提前完成 |
...
- Staged Evolution 的 promote/discard 是 Sublation 的原子操作
...
3. **Staged Evolution 是 Sublation 的安全容器** — 允许否定（修改），但在验证前不允许否定的结果逃逸到主系统
