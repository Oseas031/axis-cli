...
- **Controllable Evolution**: 策略与工具变化必须可观测、可测试、可回滚
- **Bash is All You Need, simple but robust, composable and extensible**: 首个工具即 Bash，CLI 原生，并保持错误清晰、可扩展
...
- [x] `failure_class` 三种行为正确执行
- [x] 优先级排序在 `GetReadyTasks` 中生效
- [x] `BashTool` 可执行命令并返回结果
- [x] 多轮 tool-use 循环在 `ContractExecutor` 中工作
- [x] `go test -race ./...` 通过
- [x] 覆盖率 ≥ 85%
