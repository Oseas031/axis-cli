...
Axis 不通过减少 Agent 能力来获得安全感，而是通过可查询上下文、更多行动能力、契约边界、权限阶梯和可控演化，让 Agent 能承担更复杂的任务。

### bash is all you need, simple but robust, composable and extensible

Axis 默认 shell-native。CLI、脚本、CI 和 Agent 都应能直接调用 Axis，而不需要依赖重型 UI。交互面必须保持简单、可靠、可组合、可扩展。

### Competence earns autonomy, autonomy matches responsibility, evolution is controllable

Axis 的权限系统不是静态白名单，而是递进自主权机制。Agent 通过可靠表现赢得更大的行动半径；失败、误改和验证不通过会导致自主权收缩。自主权必须匹配风险责任，权限提升、自我演化和契约重写必须经过校验。
